...
export default function Home() {
  ...
  const activeStep = useMemo(() => result?.trace.steps.at(currentStep) ?? null, [result, currentStep]);
  const controlState = useMemo(() => deriveControlState(result, currentStep), [result, currentStep]);
      ...
      <main className="grid flex-1 grid-cols-1 gap-4 p-6 lg:grid-cols-2">
        ...
        <section className="flex flex-col gap-3">
          <div className="flex h-1/2 flex-col gap-2">
            <VCFGView graph={result?.graph ?? null} activeNodeId={activeStep?.nodeId ?? null} />
