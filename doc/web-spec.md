...
理論仕様や将来計画は含めず、UI が依存するデータ構造と画面挙動を記述する。VCFG ビルダーは現在 **light モード固定**（spec-begin メタノードのみ、rollback/spec-end なし）。プロジェクト全体の俯瞰は `doc/project.md` を参照。
...
- `specWindow`: 投機長（light 固定、デフォルト 20。1 未満は `AnalysisError`）
...
（投機モードは light/discard 固定で UI オプションなし）

### 1.2 AnalysisResult スキーマ（UI が前提する形）

- 互換キー: `schemaVersion = "1.2.0"`
- `graph: StaticGraph` — ノード/エッジと種類（ns/spec）。現行は light 形態のみで、分岐ごとに `spec-begin` を 1 つ追加し、rollback/spec-end エッジは生成しない。  
